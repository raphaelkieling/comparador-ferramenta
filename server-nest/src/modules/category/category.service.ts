...
export class CategoryService {
  constructor(
    ...
    @InjectRepository(CategoryToLanguageRepository)
    private readonly categoryToLanguageRepository: CategoryToLanguageRepository,
  ...
  async create(data: CategoryCreateDTO, language: string): Promise<Category> {
    const languageEntityEn: Language = await this.languageService.getByLanguageName(
      Constants.Language.EN,
    );
    const languageEntityPt: Language = await this.languageService.getByLanguageName(
  ...
  async update(
    ...
    const translation = await this.categoryToLanguageRepository.findOneByLanguage(
      id,
      language,
    );
